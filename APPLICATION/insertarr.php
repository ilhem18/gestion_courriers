...
//ajouter données
	...
	$objet= $_POST['objet'];
	$date_arrivée= $_POST['date_arrivée'];
	    ...
	    if (!in_array($extension, ['zip', 'pdf', 'docx'])) {
	        $query="INSERT INTO courrier_arrivé(nature,emetteur,objet,date_arrivée) VALUES ('$nature', '$emetteur', '$objet', '$date_arrivée')";		
		$run=mysqli_query($db,$query);
		if($run){
		echo'<script> alert("courrier ajouté");</script>';
 		header('location: ..\admin\courrierarri.php');
		}
		else{
		echo '<script> alert("courrier non ajouté");</script>';
	        ...
	        if (move_uploaded_file($file, $destination)) {
	            $query="INSERT INTO courrier_arrivé(nature,emetteur,objet,date_arrivée,name) VALUES ('$nature', '$emetteur', '$objet', '$date_arrivée','$filename')";		
		$run=mysqli_query($db,$query);
		if($run){
		echo'<script> alert("courrier ajouté");</script>';
 		header('location:  ..\admin\courrierarri.php');
...
if (isset($_POST['update'])) {
		...
		$date_arrivée= mysqli_real_escape_string($db,$_POST['date_arrivée']);
	    ...
	    mysqli_query($db,"UPDATE courrier_arrivé SET nature='$nature', emetteur='$emetteur', objet='$objet', date_arrivée='$date_arrivée' WHERE IDA=$IDA");
		...
		mysqli_query($db,"UPDATE courrier_arrivé SET nature='$nature', emetteur='$emetteur', objet='$objet', date_arrivée='$date_arrivée',name='$filename' WHERE IDA=$IDA");
